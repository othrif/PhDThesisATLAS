
\begin{table}
\begin{center}
\setlength{\tabcolsep}{0.0pc}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lc}
\noalign{\smallskip}\hline\noalign{\smallskip}
{\bfseries Uncertainty of channel}                                    & SR0b1            \\
\noalign{\smallskip}\hline\noalign{\smallskip}
%%
Total background expectation             &  $4.30$       \\
%% \\
\noalign{\smallskip}\hline\noalign{\smallskip}
%%
Total statistical $(\sqrt{N_{\mathrm exp}})$              & $\pm 2.07$       \\
%%
Total background systematic               & $\pm 1.38\ [31.98\%] $             \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\noalign{\smallskip}\hline\noalign{\smallskip}
%%
alpha\_theoryUncertDiboson\_SR0b1         & $\pm 1.21$       \\
%%
alpha\_JET\_scale\_NP1         & $\pm 0.92$       \\
%%
alpha\_FT\_JVT         & $\pm 0.36$       \\
%%
gamma\_stat\_SR0b1\_cuts\_bin\_0         & $\pm 0.31$       \\
%%
alpha\_theoryUncertTTbarV\_SR0b1         & $\pm 0.28$       \\
%%
alpha\_theoryUncertRare\_SR0b1         & $\pm 0.27$       \\
%%
alpha\_JET\_scale\_NP3         & $\pm 0.24$       \\
%%
alpha\_FT\_B         & $\pm 0.20$       \\
%%
Lumi         & $\pm 0.18$       \\
%%
alpha\_JET\_reso         & $\pm 0.16$       \\
%%
alpha\_JET\_scale\_NP2         & $\pm 0.15$       \\
%%
alpha\_elID         & $\pm 0.06$       \\
%%
alpha\_MET\_Soft\_reso\_Perp         & $\pm 0.06$       \\
%%
alpha\_FT\_Light         & $\pm 0.05$       \\
%%
alpha\_MET\_Soft\_reso\_Para         & $\pm 0.03$       \\
%%
alpha\_muSys         & $\pm 0.03$       \\
%%
alpha\_FT\_C         & $\pm 0.03$       \\
%%
alpha\_elIso         & $\pm 0.03$       \\
%%
alpha\_elReco         & $\pm 0.02$       \\
%%
alpha\_MET\_Soft\_Scale         & $\pm 0.02$       \\
%%
alpha\_EG\_Scale         & $\pm 0.02$       \\
%%
alpha\_Mu\_MS         & $\pm 0.01$       \\
%%
alpha\_muIsoSys         & $\pm 0.01$       \\
%%
alpha\_pileupBKG         & $\pm 0.01$       \\
%%
alpha\_EG\_Resolution         & $\pm 0.01$       \\
%%
alpha\_FT\_Extra1         & $\pm 0.01$       \\
%%
alpha\_Mu\_ID         & $\pm 0.01$       \\
%%
alpha\_TRIG         & $\pm 0.01$       \\
%%
alpha\_FT\_Extra2         & $\pm 0.00$       \\
%%
alpha\_muStat         & $\pm 0.00$       \\
%%
alpha\_muIsoStat         & $\pm 0.00$       \\
%%
alpha\_JET\_EtaIntercalibration         & $\pm 0.00$       \\
%%
alpha\_Mu\_Scale         & $\pm 0.00$       \\
%%
alpha\_muSys\_lowpt         & $\pm 0.00$       \\
%%
alpha\_muStat\_lowpt         & $\pm 0.00$       \\
%%
alpha\_JET\_AFII         & $\pm 0.00$       \\
%%
\noalign{\smallskip}\hline\noalign{\smallskip}
\end{tabular*}
\end{center}
\caption[Breakdown of uncertainty on background estimates]{
Breakdown of the dominant systematic uncertainties on background estimates in the various signal regions.
Note that the individual uncertainties can be correlated, and do not necessarily add up quadratically to 
the total background uncertainty. The percentages show the size of the uncertainty relative to the total expected background.
\label{table.results.bkgestimate.uncertainties.SR0b1}}
\end{table}
\clearpage
%